  ...
  if (selectedFile) {
    ...
    fileReader.onload = function (event) {
      ...
      workbook.SheetNames.forEach((sheet) => {
        ...
        let result = parsedData.map((opa) => Object.values(opa)[columnNumber]);
        (async () => {
      ...
      <div class="c-popup l-modal__body" id="classFinder${index}">
        <div class="align-left"> Broj narudžbe: ${
          shipment.ShipmentNumber
        } </div>
        <div class="align-right"> Datum slanja: ${
          shipment.SentOnDateString
        } </div>
        ...
        <h6 class="c-popup__title">Primatelj: ${
          shipment.Consignee.PostalCode
        } ${shipment.Consignee.City}, ${shipment.Consignee.Country} </h6>
          <table class="table">
